...
content=$(cat ${POM:-'missing_pom_file'} | sed ':a;N;$!ba;s/\n//g')
...
# artifactBuiltFromOtherTag() { ( artifactOriginSourceUnknown && [ -n "$last_version_tag" ] ) && true || false }
...
for i in ${m2_items[@]}; do
   ...
   if artifactFoundInM2 ; then
      echo "Found: ${artifact}"
      msg=""
      if usingCustomTag && artifactBuiltFromUnkownSource ; then
         msg="But cannot determine if it was built from tag $version_tag as needed. Install required."
      elif usingCustomTag && artifactBuiltFromOtherTag ; then
