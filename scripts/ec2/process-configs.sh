...
getCommonName() {
  if [ "${DNS_NAME,,}" == 'local' ] || [ -z "$DNS_NAME" ] ; then
    ...
    export DNS_NAME=$(curl -s http://169.254.169.254/latest/meta-data/local-ipv4)
  else
    export DNS_NAME="${DNS_NAME}"
  fi
  echo ${DNS_NAME} | cut -d'/' -f3 # Strips off the "http://" portion and trailing "/"
}
...
processKcConfigFile() {
  # The kc-config.xml file downloaded from s3 will have placeholders for certain parameter 
  # values and need to have them looked up and filled out.
  ...
  sed -i "s/KUALI_DB_HOST/$(getDbHostname)/"            /opt/kuali/s3/kc/kc-config.xml
