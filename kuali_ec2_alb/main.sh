...
declare -A defaults=(
  ...
  [TEMPLATE_BUCKET_PATH]='s3://'$TEMPLATE_BUCKET'/cloudformation/kuali_ec2_alb'
  [TEMPLATE_PATH]='.'
...
EOF
    ...
    if [ "$action" == 'create-stack' ] ; then
      ...
      echo "Deleting any residual log groups that survived last time this stack was deleted for the same landscape..."
      aws logs delete-log-group --log-group-name /aws/lambda/${FULL_STACK_NAME}-bucket-emptier 2> /dev/null
      aws logs delete-log-group --log-group-name /aws/lambda/${FULL_STACK_NAME}-waf-logging-toggler 2> /dev/null
      aws logs delete-log-group --log-group-name /aws/lambda/${FULL_STACK_NAME}-alb-logging-toggler 2> /dev/null
    fi
